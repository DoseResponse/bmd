...
test_that("bmdOrdinal computes correct bmd estimates (def = point) for gutionS model", {
  ...
  expect_equal(guthionS.LL$pFun(10), c("alive" = 0.995187445898751, "moribund" = 0, "dead" = 0.00725091623314945), tolerance = 1e-8)
  expect_equal(guthionS.LL$pFun(20), c("alive" = 0.909311643501763, "moribund" = 0.000163464702208557, "dead" = 0.0905248917960284), tolerance = 1e-8)
  expect_equal(guthionS.LL$pFun(30), c("alive" = 0.630625814086567, "moribund" = 0.0548943439739693, "dead" = 0.314479841939464), tolerance = 1e-8)
  expect_equal(guthionS.LL$pFun(40), c("alive" = 0.327132011789939, "moribund" = 0.0972367191475815, "dead" = 0.57563126906248), tolerance = 1e-8)
  expect_equal(guthionS.LL$pFun(45), c("alive" = 0.225228290238974, "moribund" = 0.0958704725326432, "dead" = 0.678901237228382), tolerance = 1e-8)
  
  expect_equal(AIC(guthionS.LL), 222.869282843461, tolerance = 1e-8)
})
